  ...
  const logs = useMemo(() => {
    ...
    if (lastUpdated) {
      entries.push({
        ...
        date: lastUpdated,
        type: 'inventory',
        action: 'Обновление прайса',
        details: `Загружено ${items.length} позиций на витрину`,
      })
    }
    ...
    batches.forEach((b) => {
      entries.push({
        ...
        type: 'inventory',
        action: 'Создана партия',
        details: `Партия "${b.source || b.id.slice(0, 8)}" — ${b.supplier || 'без поставщика'}`,
      })
    })
    ...
    rfqs.forEach((r) => {
      entries.push({
        ...
        date: r.createdAt,
        type: 'rfq',
        action: `Заявка: ${r.title}`,
        details: `${r.items.length} позиций · Статус: ${r.status}`,
      })
    })
  ...
  return (
    <>
      <h2 className="flex items-center gap-2 text-lg font-semibold text-primary">
        <IconActivity className="shrink-0" /> Журнал активности
      </h2>
      <p className="mt-2 text-neutral-600 leading-relaxed">
        История всех действий и изменений в системе. Всего записей: {logs.length}.
      </p>

      {/* Filter */}
      <div className="mt-6 flex flex-wrap items-center gap-2">
        <span className="text-sm text-neutral-600">Тип:</span>
        {(['all', 'inventory', 'rfq', 'user', 'system'] as const).map((t) => (
          <button key={t} type="button" onClick={() => setFilter(t)} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${filter === t ? 'bg-primary text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'}`}>
            {t === 'all' ? `Все (${logs.length})` : `${TYPE_LABELS[t]} (${logs.filter((l) => l.type === t).length})`}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="mt-8 rounded-xl border-2 border-dashed border-neutral-200 bg-neutral-50 p-10 text-center">
          <p className="text-neutral-600">Нет записей активности.</p>
          <p className="mt-1 text-sm text-neutral-500">Записи появятся после загрузки прайса или создания заявок.</p>
        </div>
      ) : (
        <>
